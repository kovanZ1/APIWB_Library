...
def main() -> None:
    ...
    url = f"{BASE_URL}/api/v2/stocks-report/products/products"

    # Обязательные query-параметры: нет
    # Обязательные поля body: currentPeriod, stockType, skipDeletedNm, orderBy, availabilityFilters, offset
    # Опциональные query-параметры: нет
    # Опциональные поля body: nmIDs, subjectID, brandName, tagID, limit
    params = None
    payload = {
        "currentPeriod": {
            "start": "2026-01-01",
