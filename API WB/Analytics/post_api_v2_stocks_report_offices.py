...
def main() -> None:
    ...
    url = f"{BASE_URL}/api/v2/stocks-report/offices"

    # Обязательные query-параметры: нет
    # Обязательные поля body: currentPeriod, stockType, skipDeletedNm
    # Опциональные query-параметры: нет
    # Опциональные поля body: nmIDs, subjectIDs, brandNames, tagIDs
    params = None
    payload = {
        "currentPeriod": {
            "start": "2026-01-01",
